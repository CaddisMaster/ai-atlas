...
**Milestones 1–8 of 9 are done.** Everything but demo mode works against real
data.
...
| 9 | Demo mode — synthetic transcripts, public landing | next |
...
Demo mode passes when somebody who has never run Claude Code can clone this
repository, run one command, and see every screen filled with data — from
synthetic transcripts generated by this project, not from a recording of
anybody's real sessions.

The generated corpus has to be **honest about being synthetic**: the demo must
say so on every screen, and the numbers must not be tuned to make the tool look
insightful. A demo where the baseline is confident and every intervention has a
verdict would be a lie about what this tool does with ten real sessions.

⚠️ The generator is the first place where a fixture becomes a *product*. Every
faithfulness rule in `docs/testing.md` applies to it, and the corpus should
include the awkward cases on purpose: abandoned sessions, a subagent carrying
its parent's `sessionId`, an unmodelled record type, a compound command.
